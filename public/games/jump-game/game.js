...
function buyPremiumCoins(amount, price) {
    if (window.parent) {
...
window.addEventListener('message', (event) => {
    if (event.data.type === 'PURCHASE_SUCCESS') {
        const item = event.data.item; // "1000 Coins"
        ...
        if (!isNaN(amount)) {
            ...
            if (typeof showMessage !== 'undefined') showMessage(`코인 ${amount}개 획득!`, 'yellow', '💎');
        }
    } else if (event.data.type === 'PURCHASE_FAILED') {
        if (typeof showMessage !== 'undefined') showMessage('크레딧이 부족합니다', 'red', '❌');
        if (typeof Sounds !== 'undefined') Sounds.error();
    } else if (event.data.type === 'CAPTURE_REQUEST') {
        // 스크린샷 캡처 및 전송
        const canvas = document.getElementById('gameCanvas');
        if (canvas) {
